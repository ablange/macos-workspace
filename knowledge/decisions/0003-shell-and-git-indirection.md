...
- Load Bash from ordered fragments, then `~/.bashrc.local` last so the machine can override the portable baseline. Adopt the existing mini-data-stack-hedge-fund fragments as-is rather than redesigning them.
...
- A fresh Mac can print the Bash integration line and add a single Git include without destroying existing user configuration.
